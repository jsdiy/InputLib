...
{
	ticker.attach_ms(tickMillis, +[](HwSwitchA* me){ me->UpdateState(); }, this/*=me*/);
...
{
	SwState prevSwState = GetState();
	int8_t curentPinVal = HwSwitch::UpdateState();
	EventTrigger(curentPinVal, prevSwState);
	return curentPinVal;
}

//イベントを実行する
void	HwSwitchA::EventTrigger(int8_t pinVal, SwState prevSwState)
{
	//Clicked発生
	if (Clicked()) { onClicked.Invoke(); }

	//Held発生
	SwState currentSwState = GetState();
	if ((prevSwState == SwState::ShortHold) && (currentSwState == SwState::LongHold)) { onHeld.Invoke(); }
	...
	if (currentSwState == SwState::On || currentSwState == SwState::Release)
	{
		if (pinVal == HIGH)
		{
